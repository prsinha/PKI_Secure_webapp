...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "manageusers")) {
  $updateSQL = sprintf("UPDATE userdetails SET RID=%s, IsDeleted=%s WHERE UserID=%s",
...
mysql_select_db($database_conn, $conn);
$query_users = "SELECT UserID, CONCAT(FName,' ',MName,'. ',LName) AS  Names, RID, IsDeleted FROM userdetails";
...
<body>
<form action="<?php echo $editFormAction; ?>" method="POST" name="manageusers">
...
<td colspan="3" align="left" valign="top" style="background:url(image/top.gif);width:900px; height:96px;" ><FONT FACE="Tahoma" SIZE="6">
 ...
 <tr>
   <td width="230" align="left" valign="top"><br /><?php include('sideadminmenu.php');?></td>
...
?>
      ...
      <tr>
        <td>Delete User</td>
        <td><input type="checkbox" name="Delete" id="Delete" value="<?php echo $row_users['IsDeleted']?>" /></td>
      </tr>
      <tr>
        <td>&nbsp;</td>
        <td>
